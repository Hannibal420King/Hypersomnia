...
The build extracts `Hypersomnia-Headless.AppImage` from that stage and copies its extracted AppDir into the Vortex APP image. It makes one packaging-only change to the extracted `AppRun` shell wrapper: the final game invocation uses `exec` so the server receives container termination signals directly. The original OCI image is not used as a separately running service. This removes runtime self-extraction while retaining an auditable, immutable source for the packaged headless payload.
